...
export const Controls: React.FC<ControlsProps> = ({ params, setParams }) => {
  return (
    <div className="py-2 px-4 flex flex-wrap gap-2 items-end">
      <div className="flex-1 min-w-[180px]">
        <label className="flex items-center gap-2 text-[11px] font-bold text-slate-500 mb-1 uppercase tracking-wider">
          <Zap size={14} className="text-cyan-500" />
          Sensitivity
          <span className="ml-auto text-hit-blue tabular-nums">{params.sensitivity.toFixed(3)}</span>
        </label>
        <input
          type="range"
          min="0"
      ...
      <div className="flex-1 min-w-[180px]">
        <label className="flex items-center gap-2 text-[11px] font-bold text-slate-500 mb-1 uppercase tracking-wider">
          ...
          Noise Floor
          <span className="ml-auto text-hit-blue tabular-nums">{params.noiseFloor.toFixed(3)}</span>
        </label>
        <input
          type="range"
          min="-0.1"
      ...
      <div className="flex-1 min-w-[180px]">
        <label className="flex items-center gap-2 text-[11px] font-bold text-slate-500 mb-1 uppercase tracking-wider">
          ...
          Refractory
          <span className="ml-auto text-hit-blue tabular-nums">{params.refractory}ms</span>
        </label>
        <input
          type="range"
          min="0"
